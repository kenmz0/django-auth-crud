...
def task_detail(request, task_id):
    ...
    if request.method == 'GET':
        # tasks_for_user = Task.objects.filter(user=request.user)
        # task_user=[0]
        # for task in tasks_for_user:
        #    task_user.append(task.id)
        # if task_id > (len(task_user) - 1 ):
        #    return HttpResponseNotFound("Page Not Found 404")
        # task = get_object_or_404(Task, pk=task_user[task_id])
...
def signin(request):
    if request.method == 'GET':
        ...
        if user is None:
            return render(request, 'signin.html', {'form': AuthenticationForm, 'error': 'User or password are wrong.'})
        else:
            login(request, user)
            return redirect('tasks')
